...
def start_battle(user, *, battle):
    ...
    if battle.type == "1v1" and participant_count > 2:
        raise ValidationError("Too many participants to start the 1v1 battle")

    if battle.type == "group":
        if participant_count < 2:
            raise ValidationError("Group battles require at least 2 participants to start")
        if participant_count > battle.max_participants:
            raise ValidationError("Too many participants to start the group battle")
