...
export const useWebSocket = (options: UseWebSocketOptions) => {
  ...
  const connect = useCallback(
    (battleId: number, userId: number, username: string) => {
      clearReconnectTimer();
      intentionalCloseRef.current = false;
      connectionParamsRef.current = { battleId, userId, username };
  ...
  const attemptReconnect = useCallback(() => {
    ...
    if (reconnectAttemptsRef.current >= CONFIG.WS_RECONNECT_MAX_ATTEMPTS) {
      options.onError(
    ...
    reconnectTimerRef.current = setTimeout(() => {
      if (options.shouldReconnect()) {
        connect(params.battleId, params.userId, params.username);
      }
    }, delay);
  }, [options, connect]);
